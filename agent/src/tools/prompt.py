...
import pandas as pd
from src.schemas.future import FutureProposal, Proposal

class PromptFactory:

    @classmethod
    def create_prompt_for_merge_tables(cls, storage) -> str:
        train_columns = storage.tables_headers['train.csv']
        return f"""Посмотри структуру таблиц и определи порядок их объединения с train.csv.
    ...
    @classmethod
    def create_prompt_for_future_engineering(
            cls,
            stat,
            description,
            early_futures: Proposal | None = None,
    ) -> str:
        prompt_start = f"""
            ...
            Если нужно возвести колонку в степень 2, тогда fields=["col_name", 2] actions=["Degree"]
        ...
        if early_futures:
            print(early_futures)
            for e in early_futures:
                print(f'{e}\n\n\n')
            early_futures_str = ";".join(
                json.dumps(e.model_dump(), ensure_ascii=False, default=str)
                for e in early_futures
            )
            prompt_start += f"\nТы уже предложил следующие фичи: {early_futures_str}\nТеперь подумай, какие еще можно сделать. От тебя нужны только новые фичи"
